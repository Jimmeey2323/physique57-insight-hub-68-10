...
export const ClientConversionYearOnYearTable: React.FC<ClientConversionYearOnYearTableProps> = ({ data, onRowClick }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value: number) => (
        <div className="flex items-center justify-center gap-1">
          {(value || 0) > 0 ? <TrendingUp className="w-3 h-3 text-green-500" /> : (value || 0) < 0 ? <TrendingDown className="w-3 h-3 text-red-500" /> : null}
          <span className={`font-semibold ${(value || 0) > 0 ? 'text-green-600' : (value || 0) < 0 ? 'text-red-600' : 'text-gray-600'}`}>
            {(value || 0).toFixed(1)}%
          </span>
        </div>
      )
    ...
    },
    {
      ...
      align: 'center' as const,
      render: (value: number) => <span className="font-semibold text-green-600">{(value || 0).toFixed(1)}%</span>
    },
    {
      key: 'previousConversionRate' as const,
      header: `${new Date().getFullYear() - 1} Conv.`,
      align: 'center' as const,
      render: (value: number) => <span className="font-semibold text-gray-600">{(value || 0).toFixed(1)}%</span>
    },
    {
    ...
    },
    {
      ...
      render: (value: number) => <span className="font-semibold text-purple-600">{formatCurrency(value || 0)}</span>
    },
    {
      key: 'previousAvgLTV' as const,
      header: `${new Date().getFullYear() - 1} LTV`,
      align: 'right' as const,
      render: (value: number) => <span className="font-semibold text-gray-600">{formatCurrency(value || 0)}</span>
    },
    {
