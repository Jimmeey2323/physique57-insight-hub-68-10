...
export const ClientConversionMonthOnMonthTable: React.FC<ClientConversionMonthOnMonthTableProps> = ({
  data,
  onRowClick
}) => {
  ...
  const monthlyData = React.useMemo(() => {
    ...
    const processed = Object.values(monthlyStats).map((stat: any) => ({
      ...stat,
      trialsCompleted: stat.newMembers,
      // trials completed = new member count
      ...
      avgConversionInterval: stat.conversionIntervals.length > 0 ? stat.conversionIntervals.reduce((a: number, b: number) => a + b, 0) / stat.conversionIntervals.length : 0,
      avgVisitsPostTrial: stat.visitsPostTrial.length > 0 ? stat.visitsPostTrial.reduce((a: number, b: number) => a + b, 0) / stat.visitsPostTrial.length : 0
  ...
  const columns = [{
    ...
    render: (value: string) => <div className="font-bold text-slate-800 bg-gradient-to-r from-blue-50 to-purple-50 px-3 py-2 rounded-sm">
          {value}
        </div>
  }, {
    key: 'totalMembers',
    ...
    render: (value: number) => <div className="text-center">
          <div className="text-sm font-bold text-blue-600 px-4">{formatNumber(value)}</div>
    ...
    align: 'center' as const,
    render: (value: number) => <div className="text-center">
          <div className="text-sm font-bold text-blue-600 ml-8">{formatNumber(value)}</div>
    ...
    align: 'center' as const,
    render: (value: number) => <div className="text-center">
          <div className="text-sm font-bold text-green-700 text-center ml-8">{formatNumber(value)}</div>
    ...
    align: 'center' as const,
    render: (value: number) => <div className="text-center">
          <div className="text-sm font-bold text-purple-700 ml-8">{formatNumber(value)}</div>
    ...
    header: 'Ret. Rate',
    align: 'center' as const,
    render: (value: number) => <Badge className={`font-bold ${value > 70 ? 'bg-purple-100 text-purple-800' : value < 40 ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'}`}>
          {value.toFixed(1)}%
        </Badge>
  }, {
    key: 'totalLTV',
    ...
    render: (value: number) => <div className="text-right">
          <div className="text-sm font-bold text-emerald-600">{formatCurrency(value)}</div>
    ...
    align: 'center' as const,
    render: (value: number) => <Badge variant="outline" className="bg-orange-50 text-orange-700 border-orange-200 font-bold">
    ...
    header: 'Avg Visits',
    align: 'center' as const,
    render: (value: number) => <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 font-bold">
          {value.toFixed(1)}
        </Badge>
  }];
  ...
  return <motion.div initial={{
    ...
    duration: 0.6
      ...
      <CardContent className="p-0">
        <ModernDataTable data={monthlyData} columns={columns} headerGradient="from-blue-600 to-cyan-600" showFooter={true} footerData={totals} maxHeight="600px" onRowClick={onRowClick} />
      </CardContent>
    </Card>
    </motion.div>;
};